...
static char *get_filename_ext(const wchar_t *filename) {
	...
	char *fileExt = (char*)malloc(wcslen(filename) + 1);
	wcstombs(fileExt, dot + 1, wcslen(filename) + 1);
	return fileExt;
}
...
static int writeRSApublickey(RSA *rsa, char *key, unsigned int *keyLen, unsigned int keyMaxLen)
{
	BIO *keybio;
	unsigned int len;
	...
	{
		wprintf(L"Co loi khi ghi public key vao BIO\n");
		BIO_free(keybio);
		return 1;
	}
	len = BIO_pending(keybio);
	...
	if (BIO_read(keybio, key, len) <= 0)
	{
		wprintf(L"Co loi khi doc du lieu public key tu BIO\n");
		BIO_free(keybio);
		return 1;
	}
...
ZALO_CRYPTOLIB_API int Zalo_EncryptFile(const wchar_t *inputFileName, const wchar_t *outputFileName, unsigned int *listUserID,
...
out:
	...
	if (inputFileExt) free(inputFileExt);
	return ret;
	...
	const char *userPrivKey, unsigned int userPrivKeyLen, const char *pass)
{
	...
	ret = mbstowcs(temp, originalFileExt, originalFileExtLen+1);
	
	// gan dinh dang cho file dau ra
	len = (wcslen(outputFileName) + originalFileExtLen + 2);
	outFullFileName = (wchar_t *)malloc(len*sizeof(wchar_t));
	wmemset(outFullFileName, 0, len);
	wcscat(outFullFileName, outputFileName);
	wcscat(outFullFileName, L".");
	wcscat(outFullFileName, temp);
	//wmemcpy(outFullFileName, outputFileName, wcslen(outputFileName));
	//wmemcpy(outFullFileName + wcslen(outputFileName), L".", 1);
	//wmemcpy(outFullFileName + wcslen(outputFileName) + 1, temp, wcslen(temp));
	...
	char *pubKey, unsigned int *pubKeyLen, unsigned int pubKeyMaxLen)
{
	...
	if (!(bitlen == 2048 || bitlen == 3072 || bitlen == 4096))
	{
		return ERR_BAD_BIT_LENGTH;
	}
	// tao khoa rsa
	bne = BN_new();
	...
	if (ret != 1) {
		ret = ERR_GENERATE_KEY;
		goto out;
	}

	ret = writeRSAprivatekey(rsa, privKey, privKeyLen, privKeyMaxLen, pass);
	if (ret != 0)
	{
		wprintf(L"Co loi khi ghi khoa bi mat\n");
		if (ret == 1)
			ret = ERR_WRITE_USER_PRIVKEY;
	}
	ret = writeRSApublickey(rsa, pubKey, pubKeyLen, pubKeyMaxLen);
	if (ret != 0)
	{
		wprintf(L"Co loi khi ghi khoa cong khai\n");
		if (ret == 1)
			ret = ERR_WRITE_USER_PUBKEY;
	}
